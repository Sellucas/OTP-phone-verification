...
function App() {
  ...
  function onSignup() {
    ...
    const formathPh = "+" + ph;
    signInWithPhoneNumber(auth, formathPh, appVerifier)
      .then((confirmationResult) => {
        window.confirmationResult = confirmationResult;
        setLoading(false);
        setShowOtp(true);
        toast.success("OTP sended successufully!");
      })
      .catch((error) => {
        console.log(error);
        setLoading(false);
  ...
  return (
    <section className="bg-emerald-500 flex items-center justify-center h-screen">
      <div>
        ...
        {user ? (
          <h2 className="text-center text-white font-medium text-2xl flex gap-2">
            <span>Login Sucess</span>
            <AiFillCheckCircle size={30} className="mt-1 animate-bounce" />
          </h2>
        ) : (
